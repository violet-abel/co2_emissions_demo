...
```{python}
# 1. Melt the original `emissions` dataset. Your id_vars should be "country", your var_name should be "year" and your value_name should be "emissions".

emissions_long = emissions.melt(
    id_vars='country',      # column to keep fixed
    var_name='year',        # new column for years
    value_name='emissions'  # new column for values
)
emissions_long
...
# 2. Query for years between 1990 and 2022 (that is 1990, 1991, ..., 2021, 2022)
emissions_long_1990_2022 = emissions_long[
    (emissions_long['year'] >= 1990) & (emissions_long['year'] <= 2022)
]

# 3.  Query for 5 countries (adjust these to any countries you like)
emissions_5_countries = emissions_long_1990_2022[
    emissions_long_1990_2022['country'].isin(
        ['China', 'Nigeria', 'Canada', 'United States', 'South Africa']
    )
]

# 4. Create line chart. Year should be on the x-axis, emissions on the y-axis, and color should be by country.
fig_chart = px.line(
    emissions_5_countries,
    x='year',
    y='emissions',
    color='country',
    title='Emissions Trend (1990–2022) for Selected Countries',
    labels={
        'country': 'Country',
        'emissions': 'Emissions',
        'year': 'Year'
...
```{python}
emissions_long_1990_2022["country"] = emissions_long_1990_2022["country"].replace(
